...
import { getClubChannel, getRoleChannel, getTosChannel, getTosMcserverChannel } from "../misc/env";
...
export default define({
    async onReactionAdded(reaction: MessageReaction, user: User | PartialUser, cli: Client) {
        ...
        if (reaction.message.channel.id === getClubChannel() && user.id !== cli.user?.id) {
            // サーバーでなければ無視
            if (!reaction.message.guild) return;

            // リアクションに対応するロール定義を取得。ロールに対応しないリアクションは無視
            const r = clubs.find(r => r.emoji === reaction.emoji.toString());
            if (!r) return;
